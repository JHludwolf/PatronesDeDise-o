    ...
    public class App
    {
        ...
        public void Login()
        {
            while (true)
            {
                ...
                Console.WriteLine("Please select your user type or (3.) to View Logbook:");
                Console.WriteLine("1. Distributor");
                Console.WriteLine("2. Supplier");
                Console.WriteLine("3. View Logbook");
        ...
        public void SupplierDeliver(int SupplierID)
        {
            ...
            if(Suppliers[SupplierID].orders.Count == 0)
            {
                ...
                foreach (int DistributorID in Distributors.Keys)
                {
                    if (Distributors.ContainsKey(DistributorID))
                    {
                        Console.WriteLine($"Visiting {Distributors[DistributorID].UserName}...");
                        ...
                        Console.WriteLine($"{Distributors[DistributorID].UserName} QR Order Code scanned succesfully!");
                    }
                }
            ...
            } else
            {
                ...
                foreach (int DistributorID in sortedOrders.Keys)
                {
                    if (Distributors.ContainsKey(DistributorID))
                    {
                        if(Suppliers[SupplierID].orders.ContainsKey(DistributorID)) Console.WriteLine($"Delivering to {Distributors[DistributorID].UserName}...");
                        else Console.WriteLine($"Visiting {Distributors[DistributorID].UserName}...");
                        Console.WriteLine("Scanning Distributor QR Order Code...");
                        Suppliers[SupplierID].ScanQRCodeFromDistributor(Distributors[DistributorID]);
                        Console.WriteLine($"{Distributors[DistributorID].UserName} QR Order Code scanned succesfully!");
                    }
        ...
        public void SupplierSimulateDelivery(int SupplierID)
        {
            ...
            Logbook.Instance.LogEvent(103);
            return;
        }

        public void DistributorCaptureOrder(int DistributorID)
        {
            Logbook.Instance.LogEvent(104);
